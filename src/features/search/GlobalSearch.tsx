...
import { BookOpen, Bookmark, CheckSquare, MessageSquareText, Search, X } from "lucide-react";
...
const EMPTY_RESULTS: SearchResults = { bookmarks: [], prompts: [], journal: [] };
...
export function GlobalSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      ]).then(([bookmarks, prompts, journal]) => {
        if (!active) return;
        setResults({ bookmarks, prompts, journal });
        setStatus("idle");
      }).catch(() => active && setStatus("error"));
    }, 160);
    return () => { active = false; window.clearTimeout(timer); };
  }, [bookmarksRepository, journalRepository, open, promptsRepository, query]);

  if (!open) return null;
  const resultCount = results.bookmarks.length + results.prompts.length + results.journal.length;
    ...
    <div className="dialog-backdrop search-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <section role="dialog" aria-modal="true" aria-labelledby="global-search-title" className="global-search">
        ...
        <div className="global-search-input">
          ...
          <input ref={input} type="search" aria-label="搜索方寸" value={query} placeholder="搜索书签、提示词、随笔和 Todo…" onChange={(event) => setQuery(event.target.value)} />
